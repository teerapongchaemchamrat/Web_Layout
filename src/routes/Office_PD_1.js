...
export default function Office_PD_1() {
  ...
  const getCircle = async () => {
    try {
      const response = await axios.get("http://192.168.10.76:8080/web/pointer/location3");
  ...
  const addCircle = async (event) => {
    ...
    navigate('/register',{state:{valueX:x, valueY:y, Location:"OFFICE PD 1"}} ); //go to page register with parameter
...
const ClickableSVG = styled.svg`
  background: bisque;
  background-image: url(/image/Office_pd_1.svg?v=${uniqueIdentifier});
