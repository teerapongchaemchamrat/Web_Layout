...
export default function Factory1B() {
    ...
    const getCircle = async () => {
      try {
        const response = await axios.get("http://192.168.10.76:8080/web/pointer/location6");
    ...
    const addCircle = async (event) => {
      ...
      navigate('/register',{state:{valueX:x, valueY:y, Location:"FACTORY 1B"}} ); //go to page register with parameter
  ...
  const ClickableSVG = styled.svg`
    background: bisque;
    background-image: url(../image/Factory1B.svg);
